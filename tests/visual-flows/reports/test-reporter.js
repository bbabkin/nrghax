...
exports.EnhancedTestReporter = void 0;
const fs = __importStar(require("fs"));
const path = __importStar(require("path"));
class EnhancedTestReporter {
    constructor() {
        this.reportsDir = 'tests/reports';
        this.currentReport = {
     ...
     * Add test failure
     */
    addFailure(test, error, screenshot) {
...
<body>
  <div class="container">
    ...
    <div class="section">
      ...
      <div class="metrics-grid">
        ...
        <div class="metric-card">
          ...
          <div class="metric-value ${value >= 90 ? 'good' : value >= 50 ? 'warning' : 'error'}">
            ${value}
          </div>
        </div>
        `).join('')}
    ...
    ` : ''}
        ...
        fs.writeFileSync(path.join(this.reportsDir, 'comprehensive-test-report.html'), html);
    }
    /**
     * Update TESTING_SUMMARY_REPORT.md
     */
    async updateTestingSummaryReport() {
        const report = this.currentReport;
        const passRate = report.summary.totalTests > 0
            ? ((report.summary.passed / report.summary.totalTests) * 100).toFixed(1)
            : '0';
...
### Average Scores
${Object.entries(report.performance.averageScores || {}).map(([key, value]) => `- **${key}**: ${value} ${value >= 90 ? '✅' : value >= 50 ? '⚠️' : '❌'}`).join('\n')}

### Core Web Vitals
${report.performance.coreWebVitals ? Object.entries(report.performance.coreWebVitals).map(([key, value]) => `- **${key.toUpperCase()}**: ${value}`).join('\n') : 'Not measured'}
...
exports.EnhancedTestReporter = EnhancedTestReporter;
// Export for use in tests
exports.default = EnhancedTestReporter;
